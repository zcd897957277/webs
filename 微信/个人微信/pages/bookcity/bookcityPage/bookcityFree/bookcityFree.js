...
Page({
  ...
  data: {
    ...
    // 更多图标路径
    moreSrc:'../resources/bookcityFree/more.png',
    // 作者图标路径
    authorSrc:'../resources/images/author_03.png',
    // 倒计时 -- 限时免费
    mianfeitime_hour:'60',
    mianfeitime_minute:'60',
    mianfeitime_second:'60',
    // 倒计时 -- 章节免费
    mianfeichapters_hour: '60',
    mianfeichapters_minute: '60',
    mianfeichapters_second: '60',
    // 小说数据
    novel_free:[{
      id: 'wszz',
...
var total_micro_second = 60 * 60 * 60 * 1000;
var total_micro_second_two=30*60*60*1000;

/* 毫秒级倒计时 */
function count_down(that) {
  // 渲染倒计时时钟
  that.setData({
    mianfeitime_hour: date_format_hour(total_micro_second),
    mianfeitime_minute: date_format_minute(total_micro_second),
    mianfeitime_second: date_format_second(total_micro_second),
    mianfeichapters_hour: date_format_hour(total_micro_second_two),
    mianfeichapters_minute: date_format_minute(total_micro_second_two),
    mianfeichapters_second: date_format_second(total_micro_second_two)           
  });

  if (total_micro_second <= 0) {
    that.setData({
      mianfeitime_hour: "00",
      mianfeitime_minute: "00",
      mianfeitime_second: "00",
      mianfeichapters_hour: '00',
      mianfeichapters_minute: '00',
      mianfeichapters_second: '00'
    });
    // timeout则跳出递归
    return;
  }
  setTimeout(function () {
    ...
    total_micro_second_two -= 10;
    count_down(that);
  }, 10)
}

// 时间格式化输出，如03:25:19 86。每10ms都会调用一次
function date_format_hour(micro_second) {
  // 秒数
...
function date_format_minute(micro_second) {
  // 秒数
  var second = Math.floor(micro_second / 1000);
  // 小时位
  var hr = Math.floor(second / 3600);
...
function date_format_second(micro_second) {
  // 秒数
  var second = Math.floor(micro_second / 1000);
  // 小时位
  var hr = Math.floor(second / 3600);
